...
class PenguinDecisor(ActorDecisor):
	...
	def prolog(self,fase):
		...
		self.attackAcum = self.attackAcum-1
	...
	def newState(self, stage):
		...
		penguinPosition = self.getActor().physics.getPosition()
		nState = self.state
		...
		# del mismo y mi velocidad de salto enfuncion de la distancia que le queda por recorrer hasta darme
 		for element in stage.getPlayerShots():
			if (element.rect.bottom >= self.getActor().rect.top and element.rect.top <= self.getActor().rect.bottom):
				if (abs(element.physics.getPosition()[0]-penguinPosition[0])*element.physics.getSpeed()[0] <
		...
		if self.getActor().rect.left>=0 and self.getActor().rect.right<=ANCHO_PANTALLA:			
			...
			distanciaX =drinPosition[0]-penguinPosition[0]
			distanciaY = drinPosition[1]-penguinPosition[1]
			distancia = math.sqrt(abs(distanciaX) + abs(distanciaY))
			if (self.attackAcum >= 0):
				if (self.moveToAttack == 0):
				...
				if distancia < MELEE_VISION_RANGE*(float(stage.getLevel())/3):
					#SI ESTA DENTRO DE SU RANGO DE VISIÓN, VA A POR DRINS
					if (distanciaY < -10):
					...
					#PERSIGO A DRIN
					if drinPosition[0]>penguinPosition[0]:
						nState = self.moveRight(nState,stage)
						self.moveToAttack = 0
					elif drinPosition[0]<penguinPosition[0]:
						self.moveToAttack = 1
						nState = self.moveLeft(nState,stage)
	...
	def getMeleeClass(self, actor):
		return PenguinMelee(actor, self)
...
class PenguinInit(ActorInit):
	def __init__(self, actor):
		ActorInit.__init__(self, actor, PenguinDecisor(self))

class PenguinMelee(ActorMelee):
